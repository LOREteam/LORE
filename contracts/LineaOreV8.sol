 ...
 *
 *         V8 is a return to the atomic-resolve design of the original V7
 *         (fast, cheap UX — winner revealed inside the first bet of the
 *         next round via _autoResolveIfNeeded), with two lightweight
 *         anti-grinding tweaks appropriate for Linea's currently centralized
 *         sequencer:
 *           1. msg.sender mixed into the entropy hash — different callers
 *              see different winning tiles, so a simulate-then-submit bot
 *              must also control which address signs the resolving tx,
 *              raising the cost of adversarial shopping.
 *           2. LAST_BET_GRACE_SECONDS window at the tail of each epoch
 *              where new bets are rejected. This prevents a bettor from
 *              landing both a late bet and a resolve in the same block.
 *
 *         Economics, timelocks, rebates, jackpots, and admin API are
 *         identical to the original V7 on record.
 */
contract LineaOreV8 is Ownable2Step, ReentrancyGuard {
    ...
    // Reject bets in the final LAST_BET_GRACE_SECONDS of the epoch so that
    // a single block cannot contain both a fresh bet and the resolve tx —
    // on Linea with ~2s blocks, 2 seconds covers approximately one block.
    ...
    function _checkBetWindow() internal view {
        ...
        // Last-block anti-grinding grace: reject bets that would land in
        // the same block as the auto-resolve. Exact block detection by
        // timestamp isn't possible, so we use a short time window
        // calibrated to Linea's block cadence.
    ...
    function _resolveCurrentEpoch() internal {
        ...
        rolloverPool = 0;
        // Entropy mixes chain randomness with msg.sender so an attacker
        // who simulates the outcome must also control the sending address
        // to replay it. Combined with the LAST_BET_GRACE_SECONDS window,
        // this raises the cost of grinding on the currently-centralized
        // Linea sequencer. When Linea decentralizes this protection is
        // no longer sufficient and a contract upgrade will be required.
        L.winningTile = (
            uint256(
                keccak256(
                    abi.encodePacked(
