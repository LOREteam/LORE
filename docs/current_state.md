...
- Standalone host proof draft generation now requires saved process, `health:prod`, and `load:http` artifacts plus concrete `--db-path` and `--supervisor`; `proof:host:collect` remains the launch operator path for G5-G6 evidence collection.
...
- Standalone indexer proof draft generation now requires saved `indexer:once`, `health:prod`, and chain snapshot artifacts plus concrete deploy/start/finality inputs; `proof:indexer:collect` remains the launch operator path for G7 evidence collection.
