...
| Valkey REST-parity selection | [`valkey-upstash-parity-plan.md`](valkey-upstash-parity-plan.md) records the Valkey `8.1.9` OCI-index digest and required Linux AMD64 manifest, plus the required Upstash-compatible HTTPS REST façade. The former `837f...` reference was shown by the official manifest inspection to resolve to ARM/v7 and is rejected for this AMD64 host. Docker Engine was started, but the correct AMD64 image has not been pulled or executed because the system drive is nearly full. | Selection/design only; real HTTPS-facade, Lua, replica, and restore evidence remain open |
