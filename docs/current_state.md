...
| P1.17 native witness | The collector uses a separate, temporary headed Chromium process for the foreground visibility witness. On this Windows host, switching tabs within the measured Playwright window left its native visibility `visible`; the separate process is needed to exercise real OS/browser backgrounding. It has no route access, uses a temporary profile, and does not override visibility. | Harness repaired and self-tested only; no native-hidden or timer-throttling evidence yet |
