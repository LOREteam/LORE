...
- Restore collection now requires saved `proof:restore` and restored `health:prod` artifacts through `--restore-log` and `--health-log`; the restore log must include the successful restore summary, and the health log must include `[prod-health] OK`, matching `base=<restored-origin>`, and numeric `finalityLagBlocks` before a G8 draft is written.
...
- Host collection now requires saved `health:prod` and `load:http` logs through `--health-log` and `--load-log` before writing a G5-G6 draft; health logs must include `[prod-health] OK`, matching `base=<production origin>`, runtime/dataSync OK, and numeric `finalityLagBlocks`, while load logs must include matching `Load base URL:` plus successful bounded latency/error evidence.
...
- Indexer collection now requires saved `indexer:once`, `health:prod`, and direct-chain snapshot artifacts through `--indexer-log`, `--health-log`, and `--chain-snapshot`; the indexer log must match deploy/start/finality args, finish with `Finished runOnce`, and contain no fatal marker before a G7 draft is written.
...
- Canary proof draft generation now refuses direct writes to final `docs/canary-proof.json`, rejects generic RPC labels, and requires an existing `--live-log` JSONL artifact; final G10/G11 proof still requires real live JSONL canary evidence and strict validation.
