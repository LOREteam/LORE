...
- The goal's `318` paths describe a historical snapshot at `281c5fd02`; that
  ...
  committed. The current seven-path expected staging packet contains the P1.17
  native-window lifecycle, response-time request accounting, strict verifier
  fixtures, three current-state documents, and the self-excluded manifest. It
  is not yet staged; the user has granted local commit authority, while this
