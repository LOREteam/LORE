...
| LORE-AUD-008 | Medium | Resolve liveness | Every 120th resolve flushes owner and burn balances using external token transfers. A non-standard/frozen token or accounting insolvency would revert that resolve. | Validate the exact production token behavior and run the 120th-epoch boundary test. Avoid changing the atomic model without evidence. |
...
| LORE-AUD-025 | High | Claim discovery | Indexed cursor pagination now lets Deep Reward Scan cover the embedded wallet's full indexed participation history without sweeping every protocol epoch. Safety Pool discovery is still capped at the latest 5,000 participating epochs and has no bounded load-older UX. | Reuse the indexed candidate pages for an explicit Safety Pool load-older flow that accumulates claimable epochs and pending value in bounded on-chain chunks. Do not turn it into frequent background polling or an unbounded request. |
...
| `resolveEpoch` / auto-resolve | Resolve state, accrue liabilities, advance epoch, then optionally flush fees; whole transaction is atomic. | Only current unresolved epoch can resolve. | Known timing-selectable randomness (AUD-004) and every-120th external-transfer liveness coupling (AUD-008). |
| Single/batch reward claim | Validate eligibility/deadline, close claim state and aggregate liability, then make one transfer. | Claimed and duplicate batch epochs cannot pay twice; zero-only claims revert. Ambiguous wallet sends no longer trigger split retries. | Requires local-EVM rollback and malicious-token execution proof (AUD-015); complete one-year discovery remains open (AUD-025). |
| Single/batch rebate claim | Shared preview/consume closes user state and aggregate liability before one transfer. | Duplicate epochs skip after the first consume. Ambiguous wallet sends no longer trigger fallback retries. | Address-based eligibility is sybil-asymmetric (AUD-011); complete one-year discovery remains open (AUD-025). |
