...
async function main() {
  ...
  try {
    ...
    assert.equal(safeSoakStatus.diskCapacity.diskFreeMinimumBytes, 1_073_741_824);
    ...
    assert.equal(safeSoakSummary.diskCapacity.diskFreeMinimumBytes, 1_073_741_824);
    ...
    const lowDiskSoakSummaryResult = spawnSync(process.execPath, ["scripts/run-testnet-soak-supervisor.mjs", "--status", "--summary-only"], {
      cwd: process.cwd(),
      env: { ...process.env, SOAK_OUT_DIR: soakStatusDir, SOAK_MIN_DISK_FREE_BYTES: String(Number.MAX_SAFE_INTEGER) },
      encoding: "utf8",
    });
