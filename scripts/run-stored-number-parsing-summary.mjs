...
export function summarizeStoredNumberParsingResult(result) {
  ...
  return {
    status: pass ? "pass" : "fail", storedNumberParsing: true, passed, assertionFailures, timedOut,
...
export function runStoredNumberParsingSummary({
  ...
  const args = env.npm_execpath
    ? [env.npm_execpath, "--silent", "run", "test:stored-number-parsing"]
    : ["--silent", "run", "test:stored-number-parsing"];
  const result = spawn(command, args, {
    cwd, encoding: "utf8", maxBuffer: 512 * 1024,
    timeout: parseSummaryTimeoutEnv("STORED_NUMBER_PARSING_SUMMARY_TIMEOUT_MS", 60_000),
    env: { ...env, NO_UPDATE_NOTIFIER: "1", npm_config_update_notifier: "false", npm_config_fund: "false" },
  });
  const summary = summarizeStoredNumberParsingResult(result);
...
if (isDirectRun()) {
  try {
    const { exitCode } = runStoredNumberParsingSummary();
    process.exitCode = exitCode;
  } catch (error) {
    console.log(JSON.stringify({
      status: "fail", issue: "stored-number-parsing-summary-config-invalid",
