...
const artifactBackedDocExpectations = [
  [
    ...
    [
      ...
      ["proof:restore:collect", ["--restore-log=docs/restore-drill.log", "--health-log=docs/restore-health-prod.log", "--backup-schedule-artifact=docs/restore-backup-schedule.log", "--preservation-artifact=docs/restore-indexer-preservation.log"]],
...
assertOrderedMarkers("docs/production-runbook.md", productionRunbook, [
  ...
  ["dependency audit", "npm.cmd run proof:deps"],
  ["full dependency audit", "npm.cmd run proof:deps:all"],
