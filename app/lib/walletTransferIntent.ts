...
export type WalletTransferReceiptClient = {
...
const LOCK_PREFIX = "lineaore:wallet-transfer-intent-lock:v1";
...
function lockKey(intent: WalletTransferIntent) {
  return `${LOCK_PREFIX}:${intent.chainId}:${intent.actor}`;
}
...
function assertNoConflictingActorState(intent: WalletTransferIntent, now: number) {
  ...
  try {
    actorKeys = Array.from({ length: storage.length }, (_, index) => storage.key(index))
      .filter((key): key is string => key !== null && key.startsWith(actorKeyPrefix));
  } catch {
    throw intentError("wallet_transfer_intent_storage_read_failed");
  }
...
function writeState(intent: WalletTransferIntent, state: WalletTransferIntentState, now: number) {
  ...
  if (!persisted || persisted.id !== state.id || persisted.hash !== state.hash) {
...
async function withIntentLock<T>(
  ...
  if (typeof navigator === "undefined" || !navigator.locks) {
    throw intentError("wallet_transfer_intent_web_lock_unavailable");
  }
  return navigator.locks.request(
    lockKey(intent),
    ifAvailable ? { mode: "exclusive", ifAvailable: true } : { mode: "exclusive" },
    async (lock) => {
      if (!lock) throw intentError("wallet_transfer_intent_locked");
      return callback();
    },
  );
}
...
async function assertStableWalletTransferReceipt(
  ...
  const second = await readWalletTransferReceiptQuorum(clients, hash);
  if (
    !second ||
    receiptFingerprint(second, hash) !== firstFingerprint
  ) {
    throw intentError("wallet_transfer_receipt_diverged", hash);
  }
