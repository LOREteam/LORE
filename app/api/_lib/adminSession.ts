...
function getSessionSecret() {
  ...
  if (!missingSecretWarningShown) {
    ...
    console.warn("[admin-session] Using a deterministic development fallback secret. Set ADMIN_AUTH_SECRET to match production behavior.");
  }

  return createHash("sha256")
    .update(`dev-admin-session:${process.cwd()}`)
    .digest("hex");
}
